...
# 收盘价主源(东财)取不到时是否转新浪兜底。东财会按 IP 封禁且一封就是整批取不到，
# 新浪收盘价与东财逐位一致(均不复权)，故默认开启；若哪天新浪改口径(如变复权)
# 导致价格跳变，把这里关掉即可退回单源。
PRICE_FALLBACK_SINA = True
